...
//        parent::__construct();
        ...
        //查询购物车的信息
        $this->Mcartlist = _getcookie("Mcartlist");
    }
...
//        var_dump($state);
        ...
        if ($state == -1 || $state == 1) {
            ...
            $records = rpc_records_getActiveRecords();

            $goodsIds = [];
            foreach ($records as $record) {
                $goodsIds [] = $record['GoodsId'];
            }
            $shoplist = rpc_mall_getGoodsInfos($goodsIds);
            $shoplistall['listItems'] = $shoplist;
...
//        \hellaEngine\support\dump($shoplistall);
    ...
    public function getUserOrderList()
    {
        ...
        $records = rpc_records_getAcceptRecords($FIdx, $EIdx);

        $itemIds = [];

        $recordsByGoodsId = [];
        foreach ($records as $record) {
            $itemIds [] = $record['goodsId'];
            $recordsByGoodsId[$record['goodsId']] = $record;
        }

        $items = rpc_mall_getGoodsInfos($itemIds);

//        var_dump($items);

        foreach ($items as $item) {
            $item['record'] = $recordsByGoodsId[$item["id"]];
        }

        $orderlist = [];
        $orderlist['listItems'] = $items;//$this->db->GetList("select * from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where b.q_uid='$member[uid]' and a.huode!=0 order by a.time desc limit $FIdx,$EIdx ");
        $orderlist['count'] = count($items);
        $orderlist['code'] = 0;
        // echo "<pre>";
        // print_r($orderlist);
