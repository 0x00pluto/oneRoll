...
//		}
    ...
    {
        $webname = $this->_cfg['web_name'];
        $mysql_model = System::load_sys_class('rpcmodel');
        $member = $this->userinfo;
        $uid = $member['uid'];
        $title = "夺宝记录";
        $user_dizhi = rpc_records_getActiveRecords();
...
//        $paylist = $this->db->GetList("SELECT * FROM `@#_pay` where `pay_start` = '1' AND pay_mobile = 1");
    ...
    public function useraddress()
    {
        ...
        if ($_POST['submit']) {
            ...
            if (empty($skfs) || empty($skr) || empty($skzh)) {
                _messagemobile("收款方式 收款人 收款帐号不能为空", WEB_PATH . "/mobile/home/address");
            }
            $q1 = $this->db->Query("INSERT INTO `@#_member_dizhi` SET `time` = $t, `uid` = $uid, `sheng` = '$sheng', `shi` = '$shi', `xian` = '$xian', `jiedao` = '$jiedao',`youbian` = '$youbian', `shouhuoren`= '$shouhuoren', `mobile`= '$mobile', `qq`= '$qq', `skfs`= '$skfs', `skr`= '$skr', `skzh`= '$skzh', `shifoufahuo` = $shifoufahuo");
    ...
    public function updateddress()
    {
        ...
        if ($id) {
            ...
            /*
            if(empty($qq) || empty($youbian) || empty($shouhuoren) || empty($mobile) || empty($skfs) || empty($skr) || empty($skzh)){

                _messagemobile("qq 或者 邮编 收货人 电话 收款方式 收款人 收款帐号不能为空",WEB_PATH."/mobile/home/address");
            }
*/
            if (empty($skfs) || empty($skr) || empty($skzh)) {
                _messagemobile("收款方式 收款人 收款帐号不能为空", WEB_PATH . "/mobile/home/address");
            }
            $q1 = $this->db->Query("UPDATE `@#_member_dizhi` SET `time` = $t, `sheng` = '$sheng', `shi` = '$shi', `xian` = '$xian', `jiedao` = '$jiedao',`youbian` = '$youbian', `shouhuoren`= '$shouhuoren', `mobile`= '$mobile', `qq`= '$qq', `skfs`= '$skfs', `skr`= '$skr', `skzh`= '$skzh', `shifoufahuo` = '$shifoufahuo' WHERE `id`= $id");
