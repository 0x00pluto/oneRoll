...
function pay_insert_shop($shop='',$type=''){
	...
	if($shop['zhiding'] && $shop['zhiding'] != $u_go_info['id'] ){
		...
		if($ex_code[$ex_rand]){
			...
			$str=str_replace($code,$ex_code[$ex_rand],$tempinfo['goucode']);

			$db->Query("update `@#_member_go_record` set goucode='$str' where id='{$tempinfo['id']}'");

			//将系统原来的中奖吗对应的购买记录换成指定中奖会员购买的code

			$str2=str_replace($ex_code[$ex_rand],$code,$ex_info['goucode']);
	...
	/*var_dump($u_go_info);
	var_dump("select * from `@#_member_go_record` where `shopid` = '$shop[id]' and `shopqishu` = '$shop[qishu]' and `goucode` LIKE  '%$code%'");
	var_dump("select uid,username,email,mobile,img from `@#_member` where `uid` = '$u_go_info[uid]'");
	exit("111");*/
