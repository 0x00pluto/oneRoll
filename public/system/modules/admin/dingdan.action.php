...
class dingdan extends admin {
	...
	private function send_wx_ship_code($gid=null){
		...
		$user_dizhi = $this->db->GetOne("SELECT * FROM `@#_member_dizhi` where `uid` = '{$info['q_uid']}' ORDER BY `default`  DESC LIMIT 1");
		if(!empty($member_band['b_code'])){
			...
			$data = array(
				...
				"data" => array(
					...
					"keyword4"=>array(
						"value"=>$user_dizhi['sheng'].' - '.$user_dizhi['shi'].' - '.$user_dizhi['xian'].' - '.$user_dizhi['jiedao']."(".$user_dizhi['shouhuoren']."收)",
						"color"=>"#173177",
						),
					"remark"=>array(
