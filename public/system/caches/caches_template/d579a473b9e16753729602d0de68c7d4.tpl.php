...
<style type="text/css">
	*{margin:0;padding:0;}
	...
	.box{ border:5px solid #eee; width:480px; background-color:#fff;border-radius:10px;
		  margin-top:15%;erflow:hidden;		
	}
	.box-b{ border:1px solid #dfdbdb;width:478px;erflow:hidden;border-radius:5px;}
	.box-title{border-radius:5px 5px 0 0;background:<?php if(isset($config['titlebg'])): ?><?php echo $config['titlebg']; ?><?php  else: ?>#e71c46<?php endif; ?>;height:30px;line-height:30px;_line-height:30px;font-size: 12px;color:<?php if(isset($config['title'])): ?><?php echo $config['title']; ?><?php  else: ?>#fff<?php endif; ?>; padding:0 10px;}
	.box-text{font:12px/1.5 "微软雅黑",Arial,"宋体",Helvetica,sans-serif; font-size:14px;color:#73787b;width:438px;		
	...
	.box-button a{  display: inline-block;					
					height:25px;line-height:25px;_line-height:25px;
					text-align: center;
					font-family:"微软雅黑";
					font-size: 12px;
