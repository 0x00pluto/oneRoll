...
class wechatCallbackapiTest
{
	...
	public function responseMsg(){
			...
			if ($postObj->MsgType == 'event') {
				...
				if ($postObj->Event =="subscribe"){
					...
					if(empty($retuser)){  //如果用户不存在
						...
						$contentreg = "\n恭喜您，关注本账号即注册成功!\n您的用户名为:".$nickname."\n密码为:".$password."\n\n<a href='".$base_url."index.php/mobile/home'>点此进入会员管理中心，点击底部菜单领取体验红包赶紧去抢购心爱的商品吧！</a>";
						//如果是二维码关注场景--S
		...
		if($keyword == 'debug'){
			...
			$msgType = "text";
			$contentStr = "<a href='".$base_url."index.php/mobile/home'>点此进入会员管理中心，您也可以点击下方菜单会员进入</a>";
			$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
			echo $resultStr;
			$this->autologin($fromUsername);
			exit;
			...
			if ($jf_state == 'yes') {
				...
				$contentStr = '尊敬的用户，非常抱歉，签到送福分已停止使用，可能您来晚了，请您继续关注我们的消息，再次开启此活动您会收到及时通知的哟！';
			}
			$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
			echo $resultStr;
			exit;
			...
			if(empty($bind)){
				$contentStr = "信息获取失败！请进入网站选择快捷登录或重新绑定您的微信账号到网站（建议您先用微信快捷登陆或者重新关注微信公众号），然后再操作。";
			}else{
				$uid = $bind['b_uid'];
				if($uid<1){
					$contentStr = "用户信息错误！";
			...
			if(empty($ret)){
				$contentStr = "用户信息错误，建议您取消关注后重新关注哦！";
				$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
				echo $resultStr;
				exit;
			}
			...
			if (!empty($orders)) {
				...
				$msgType = "text";
				$contentStr = "亲，非常抱歉，系统还没有您的中奖订单，不要灰心哟，机会肯定会到来的！";
				$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
				echo $resultStr;
			}
			exit;
			...
			if (empty($orders)) {
				$msgType = "text";
				$contentStr = '尊敬的用户，当前您还没有中奖订单，无法查询快递信息';
				$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
				echo $resultStr;
				exit;
			}
			if (empty($orders['company_code'])) {
				$msgType = "text";
				$contentStr = '尊敬的用户，您的中奖订单：' . $orders['shopname'] . '目前还没有快递单号，不能查询';
				$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
				echo $resultStr;
				exit;
			}
			...
			if($ArticleCount >= 1){
				...
				$contentStr = 'sorry，没有帮您搜索到"' . $goods_name . '"' . $tj_str;
				$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
				echo $resultStr;
				$this->autologin($fromUsername);
				exit;
	...
	protected function coupon($uid) {
		...
		if($res['status']>0){
			return $contentStr = "亲，您已经领取过现金了,不能重复领取的哦！\r\n您领取红包的时间为".date('Y-m-d H:i:s',$res['input_time']);
		}
		//查询设置的红包类型
		$ret = $this->db->GetOne("SELECT * FROM `@#_weixin_bonus` WHERE `id` = 1");
		$type_id = $ret['type_id'];
		if($type_id == 0){
			return '非常抱歉领，红包功能暂时没有开启，请持续关注我们推送的消息，抢得大红包！';
		}
		...
		if (($time >= $rets['send_start_date']) && ($time <= $rets['send_end_date'])) {
			if ($rets['total']>0 && ($rets['total']-$rets['getnum']) >0) {
				...
				$contentStr = "尊敬的用户，很高兴地通知您，您获得了本站赠送的". $type_money . "元体验红包\r\n现在已经存入到您的账户\r\n可以用来购买商品哦！祝您好运，中得大奖！";
			} else {
				$contentStr = "呀，亲，真是遗憾！您来晚了哦！\r\n本次活动红包已送完!";
			}
		}else{
			$contentStr = "呀，亲，真是遗憾！您来晚了哦！\r\n目前不在红包活动时间内或者活动时间已经过期，请您耐心等待，继续关注消息提示";
		}
		return $contentStr;
	}
	...
	public function plusPoint($uname, $keyword, $fromUsername) {
		...
		if (empty($record)) {
			...
			if ($record_num['num'] < 1) {
				...
				$res_arr['errmsg'] = 'no';
				$res_arr['contentStr'] = '亲，感谢您持续签到，但是您今天已经签过到过了啦，明天继续吧！祝您好运，大奖肯定是您的哟！';
				return $res_arr;
			}
		}
		...
		if($record_num['total']==0){
			$res_arr['contentStr'] = "尊敬的用户，您今天签到成功,\r\n目前共累计签到1天 , 累计所得福分为+";
		}
		if($record_num['total']>0){
			$totals = $record_num['total']+1;
			$res_arr['contentStr'] ="亲，您共签到".$totals."天\r\n上次签到时间是：".date('Y-m-d H:i:s',$record_num['lasttime']) ."\r\n本次签到成功,福分+";
		}
		return $res_arr;
	}
