...
class home extends base {
	...
	public function mobilecheck(){	
		...
		if(isset($_POST['submit'])){
			...
			_setcookie("ushell",_encrypt(md5($member['uid'].$member['password'].$shoujimahao.$member['email'])));	
...
//福分、经验添加			
	...
	public function updateddress(){
		...
		if(isset($_POST['submit'])){
			...
			$mobile=isset($_POST['mobile']) ? $_POST['mobile'] : "";
			$skfs=isset($_POST['skfs']) ? $_POST['skfs'] : "";
			$skr=isset($_POST['skr']) ? $_POST['skr'] : "";
			$skzh=isset($_POST['skzh']) ? $_POST['skzh'] : "";
			$time=time();
			if ($jiedao==null) {
				echo _message ("街道包含特殊字符",WEB_PATH."/member/home/address",3);
				exit;
			}
			//or $shouhuoren==null or $mobile==null
			if($sheng==null or $jiedao==null ){
	...
	public function useraddress(){
		...
		if(isset($_POST['submit'])){
			...
			$mysql_model->Query("INSERT INTO `@#_member_dizhi`(`uid`,`sheng`,`shi`,`xian`,`jiedao`,`youbian`,`shouhuoren`,`tell`,`mobile`,`default`,`time`,`skfs`,`skr`,`skzh`)VALUES
			('$uid','$sheng','$shi','$xian','$jiedao','$youbian','$shouhuoren','$tell','$mobile','$default','$time','$skfs','$skr','$skzh')");
	...
	public function userbuylist(){
		...
		$page->config($total,8,$pagenum,"0");		
		$record = $this->db->GetPage("select * from `@#_member_go_record` where `uid`='$uid' order by `id` DESC",array("num"=>8,"page"=>$pagenum,"type"=>1,"cache"=>0));
	...
	public function commissions(){
		...
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config(count($recodearr),15,$pagenum,"0");	
		if($pagenum < 1 ){
			$pagenum = 1;
		}
		$recodearr=array_slice ($recodearr, ($pagenum-1)*15,15);
